...
const dataProvider = new BinanceDataProvider();
...
it('checks if symbol can be changed and onSymbolChange and onIntervalChange get called correctly', async () => {
  const handleSymbolChange = jest.fn();
  const handleIntervalChange = jest.fn();
  dataProvider.fetchSymbols.mockResolvedValue(LIST_OF_SYMBOLS);
  act(() => {
...
it('checks if interval can be changed and onSymbolChange and onIntervalChange get called correctly', async () => {
  const handleSymbolChange = jest.fn();
  const handleIntervalChange = jest.fn();
  dataProvider.fetchSymbols.mockResolvedValue(LIST_OF_SYMBOLS);
  act(() => {
